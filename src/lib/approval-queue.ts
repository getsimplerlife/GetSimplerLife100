...
const WRITE_VERB = /^(create|update|delete|remove|send|upload|copy|move|write|post|set|mark|complete|void|trash|ingest|trigger|submit|approve|reject|cancel|add|edit|archive|restore|rename|invite|assign|schedule|execute|generate|import|export|extract|start|stop|pause|resume|open|close|accept|decline|reopen|publish|confirm|request|run|activate)/i;
