...
describe("Microsoft Excel client (Graph)", () => {
  ...
  it("createExcelWorkbook builds a valid xlsx and PUTs to OneDrive", async () => {
    ...
    expect(captured!.url).toBe("https://graph.microsoft.com/v1.0/me/drive/root:/Ledger.xlsx:/content");
  ...
  it("readWorkbookRange GETs the workbook API range object", async () => {
    const c = makeClient();
    const calls: string[] = [];
    ...
    expect(calls[0]).toContain("/me/drive/items/wb-1/workbook/worksheets/Sheet1/range(address='A1:B2')");
    expect(calls[0]).not.toContain("/values");
    expect(values).toEqual([["a", "b"], ["c", "d"]]);
  });

  it("readWorkbookRange falls back to the range text when values is absent", async () => {
    const c = makeClient();
    (c as any).client.get = async () => ({ data: { text: [["x"]] } });
    const values = await c.readWorkbookRange("wb-1", "A1");
  ...
  it("writeWorkbookRange PATCHes the range object with values", async () => {
    const c = makeClient();
    ...
    expect(calls[0].path).toContain("/me/drive/items/wb-1/workbook/worksheets/Sheet1/range(address='A1')");
    expect(calls[0].path).not.toContain("/values");
