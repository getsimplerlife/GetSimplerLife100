...
export const customerSupportCapabilities: ReadonlyArray<CapabilityContract> = [
  ...
  defineCapabilityContract({ employeeId: CUSTOMER_SUPPORT_EMPLOYEE_ID, capabilityId: "zendesk-read-ticket-fields", kind: "understand", status: "unverified", providerId: ZENDESK_PROVIDER_ID, tenantScoped: true, authRequired: true, auditRequired: true, idempotencyRequired: false, retryPolicy: "bounded", rollback: "not_applicable", evidence: "Provider adapter capability path exists; authorized tenant evidence is pending." }),  defineCapabilityContract({ employeeId: CUSTOMER_SUPPORT_EMPLOYEE_ID, capabilityId: "zendesk-update-ticket-status", kind: "automate", status: "unverified", providerId: ZENDESK_PROVIDER_ID, tenantScoped: true, authRequired: true, auditRequired: true, idempotencyRequired: true, retryPolicy: "bounded", rollback: "available", evidence: "Provider adapter capability path exists; authorized tenant evidence is pending." }),  defineCapabilityContract({ employeeId: CUSTOMER_SUPPORT_EMPLOYEE_ID, capabilityId: "zendesk-read-knowledge-base", kind: "understand", status: "unverified", providerId: ZENDESK_PROVIDER_ID, tenantScoped: true, authRequired: true, auditRequired: true, idempotencyRequired: false, retryPolicy: "bounded", rollback: "not_applicable", evidence: "Provider adapter capability path exists; authorized tenant evidence is pending." }),];
