...
export const vaultCapabilities: ReadonlyArray<CapabilityContract> = [
  ...
  defineCapabilityContract({
    employeeId: DOCUMENT_VAULT_EMPLOYEE_ID,
    capabilityId: "vault-folder-management",
    kind: "automate",
    status: "real",
    providerId: VAULT_PROVIDER_ID,
    tenantScoped: true,
    ...
    retryPolicy: "bounded",
    rollback: "available",
    evidence: "Phase 1.5d FILING LAYER — per-tenant structured LOCATIONS: explicit folder create/list/rename/labels/delete over the same route DSL as filing (canonicalizeRoute — never a guessed path), non-destructive by construction (a folder with sub-folders or filed documents can never be renamed/deleted — no orphaned routes), exact-id deletes with idempotent-by-audit replay, every mutation in the immutable vault audit (vault.folder.create/.rename/.delete/.labels), and cross-tenant ids fail closed (Folder not found). This is catalog metadata (parallel to the 5c template catalog) — document writes stay approval-gated in the vault write floor. LIVE-PROVEN 09-18 twice (deployed e7437fa, prod PID 11107): smoke tenant r33a-1789753143@test.local — POST /api/vault/folders {path:\"R33/Contracts/2026\", labels:[round33,filing]} → 3 nodes (R33, R33/Contracts, leaf 2026 with labels) → GET /api/vault/folders lists all paths + leaf labels → duplicate exact path → 400 Folder already exists → rename leaf fol_e93bc459463a → R33/Contracts/Agreements → labels POST → [signed,fy2026] → DELETE id → {ok:true} → re-DELETE same id → {ok:true, unchanged:true} (idempotent-by-audit replay) → unknown id → 400. Cross-tenant fail-closed (tenant r33b-1789753143@test.local): GET → empty taxonomy; B rename/delete of A's exact folder id → 400 Folder not found. Unauthenticated GET/POST /api/vault/folders → 401. Deployed instance's own vault_audit.json: tenant A records vault.folder.create ×3, vault.folder.rename ×1, vault.folder.labels ×1, vault.folder.delete ×2 (delete + audited replay); tenant B → ZERO entries. Smoke stores cleaned post-verify (A folders:0 rules:0).",
  }),
  defineCapabilityContract({
    employeeId: DOCUMENT_VAULT_EMPLOYEE_ID,
    capabilityId: "vault-folder-rules",
    kind: "automate",
    status: "real",
    providerId: VAULT_PROVIDER_ID,
    tenantScoped: true,
    ...
    retryPolicy: "bounded",
    rollback: "available",
    evidence: "Phase 1.5d FILING LAYER — auto-folder TAXONOMY rules: per-tenant rule create/update/delete (exact id, never glob), enabled toggle, strict target validation (unknown placeholders rejected at save — never a guessed route), pure route prediction (applyAutoRoute) feeding the approval-gated vault-document.file write, and every rule mutation lands an immutable vault.folder.rule.create/.update/.delete audit entry (closing the pre-5d gap where rule CRUD was un-audited). Rules live in the tenant-scoped folder store — cross-tenant rule ids fail closed. LIVE-PROVEN 09-18 twice (deployed e7437fa, prod PID 11107): smoke tenant r33a-1789753143@test.local — POST /api/vault/folders/rules {name:\"R33 by customer\", enabled:true, dimensions:[customer], target:\"{customer}/Docs\", priority:77} → rule_ce7fb78c2297 → GET /api/vault/folders lists it in data.rules → update same id (name v2, target {customer}/Docs/2, enabled:false) → applied in place → DELETE id → 200. Cross-tenant fail-closed (tenant r33b-*): DELETE A's rule id → 404 Rule not found; B's GET → empty rules. Deployed instance's own vault_audit.json: tenant A records vault.folder.rule.create ×1, vault.folder.rule.update ×1, vault.folder.rule.delete ×1; tenant B → ZERO entries. Smoke stores cleaned post-verify.",
  }),
];