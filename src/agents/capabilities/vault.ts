 ...
 * (this branch feat/vault-verified-5ab — executor commits b5aa0af/bdac1eb —
 * registers writeVaultDocument / moveVaultDocument / archiveVaultDocument /
 * deleteVaultDocument in the engine action registry)
...
export const vaultCapabilities: ReadonlyArray<CapabilityContract> = [
  ...
  defineCapabilityContract({
    ...
    retryPolicy: "bounded",
    rollback: "available",
    evidence: "Re-routes an existing vault document; approval-gated, idempotent, audited. LIVE-PROVEN 09-11 (deployed ab69e5c): pending-gating live-proven (approval queue + audit pending entry). HARNESS-PROVEN: approve→execute proven end-to-end with the native vault executors on this branch. FLIP GATE: flips to verified after this branch (feat/vault-verified-5ab) merges + deploys AND a live approve→execute smoke on the deployed instance passes.",
  }),
  defineCapabilityContract({
    employeeId: DOCUMENT_VAULT_EMPLOYEE_ID,
    ...
    retryPolicy: "bounded",
    rollback: "available",
    evidence: "Retention-flag transition to archived; approval-gated; restore available. LIVE-PROVEN 09-11 (deployed ab69e5c): pending-gating live-proven (approval queue + audit pending entry). HARNESS-PROVEN: approve→execute proven end-to-end with the native vault executors on this branch. FLIP GATE: flips to verified after this branch (feat/vault-verified-5ab) merges + deploys AND a live approve→execute smoke on the deployed instance passes.",
  }),
  defineCapabilityContract({
    employeeId: DOCUMENT_VAULT_EMPLOYEE_ID,
