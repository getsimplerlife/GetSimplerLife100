...
export const vaultCapabilities: ReadonlyArray<CapabilityContract> = [
  ...
  defineCapabilityContract({
    ...
    retryPolicy: "bounded",
    rollback: "available",
    evidence: "Phase 1.5d FILING LAYER — per-tenant structured LOCATIONS: explicit folder create/list/rename/labels/delete over the same route DSL as filing (canonicalizeRoute — never a guessed path), non-destructive by construction (a folder with sub-folders or filed documents can never be renamed/deleted — no orphaned routes), exact-id deletes with idempotent-by-audit replay, every mutation in the immutable vault audit (vault.folder.create/.rename/.delete/.labels), and cross-tenant ids fail closed (Folder not found). This is catalog metadata (parallel to the 5c template catalog) — document writes stay approval-gated in the vault write floor. SHIPPED on branch feat/vault-5d-filing (PR pending) with lib + API + isolation tests green (canonical suite). Live verification (deployed-sha smoke: create → rename → labels → delete → replay + cross-tenant probes + audit corroboration) is pending lead merge + deploy — status flips to real in the verification round that cites ONLY the deployed sha.",
  }),
  defineCapabilityContract({
    employeeId: DOCUMENT_VAULT_EMPLOYEE_ID,
    ...
    retryPolicy: "bounded",
    rollback: "available",
    evidence: "Phase 1.5d FILING LAYER — auto-folder TAXONOMY rules: per-tenant rule create/update/delete (exact id, never glob), enabled toggle, strict target validation (unknown placeholders rejected at save — never a guessed route), pure route prediction (applyAutoRoute) feeding the approval-gated vault-document.file write, and every rule mutation now lands an immutable vault.folder.rule.create/.update/.delete audit entry (closing the pre-5d gap where rule CRUD was un-audited). Rules live in the tenant-scoped folder store — cross-tenant rule ids fail closed. SHIPPED on branch feat/vault-5d-filing (PR pending) with lib + API + isolation tests green (canonical suite). Live verification (deployed-sha smoke: rule create → list → update → delete + cross-tenant probes + audit corroboration) is pending lead merge + deploy — status flips to real in the verification round that cites ONLY the deployed sha.",
  }),
];