...
export async function readOrders(adapter: Pick<ManufacturingAdapter, "listOrders">, options: ManufacturingExecutionOptions): Promise<unknown> { requireTenant(options); let lastError: unknown; for (let attempt=0; attempt<boundedAttempts(options.maxAttempts); attempt++) { try { const result=await adapter.listOrders(options.tenantId); await options.audit({capabilityId:"shopify-read-orders",tenantId:options.tenantId,outcome:"succeeded"}); return result; } catch(error) { lastError=error; } } await options.audit({capabilityId:"shopify-read-orders",tenantId:options.tenantId,outcome:"failed"}); throw lastError; }
export async function createProduct(adapter: Pick<ManufacturingAdapter, "createProduct">, input: Record<string, unknown>, options: ManufacturingExecutionOptions, idempotencyKey: string): Promise<unknown> { requireTenant(options); if (!idempotencyKey.trim()) throw new Error("Idempotency key is required"); let lastError: unknown; for (let attempt=0; attempt<boundedAttempts(options.maxAttempts); attempt++) { try { const result=await adapter.createProduct(options.tenantId,input,idempotencyKey); await options.audit({capabilityId:"shopify-create-product",tenantId:options.tenantId,outcome:"succeeded",idempotencyKey}); return result; } catch(error) { lastError=error; } } await options.audit({capabilityId:"shopify-create-product",tenantId:options.tenantId,outcome:"failed",idempotencyKey}); throw lastError; }
