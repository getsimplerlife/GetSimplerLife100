...
export class JotformClient {
  ...
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.jotform.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createJotformClient(config: ConnectionConfig): JotformClient {
  const c = new JotformClient(config.apiKey || "");
