...
export class Gravity_formsClient {
  private client: HttpClient;
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.gravity-forms.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createGravity_formsClient(config: ConnectionConfig): Gravity_formsClient {
  const c = new Gravity_formsClient(config.apiKey || "");
