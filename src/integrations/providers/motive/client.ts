...
export class MotiveClient {
  ...
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.motive.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createMotiveClient(config: ConnectionConfig): MotiveClient {
  const c = new MotiveClient(config.apiKey || "");
