...
export class Google_geminiClient {
  private client: HttpClient;
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.google-gemini.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createGoogle_geminiClient(config: ConnectionConfig): Google_geminiClient {
  const c = new Google_geminiClient(config.apiKey || "");
