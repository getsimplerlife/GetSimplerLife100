...
export class Google_formsClient {
  private client: HttpClient;
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.google-forms.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createGoogle_formsClient(config: ConnectionConfig): Google_formsClient {
  const c = new Google_formsClient(config.apiKey || "");
