...
export class FishbowlClient {
  ...
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.fishbowl.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createFishbowlClient(config: ConnectionConfig): FishbowlClient {
  const c = new FishbowlClient(config.apiKey || "");
