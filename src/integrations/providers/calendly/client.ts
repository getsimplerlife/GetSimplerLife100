...
export class CalendlyClient {
  ...
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.calendly.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createCalendlyClient(config: ConnectionConfig): CalendlyClient {
  const c = new CalendlyClient(config.apiKey || "");
