...
export class CohereClient {
  ...
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.cohere.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createCohereClient(config: ConnectionConfig): CohereClient {
  const c = new CohereClient(config.apiKey || "");
