...
export class IqmsClient {
  ...
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.iqms.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createIqmsClient(config: ConnectionConfig): IqmsClient {
  const c = new IqmsClient(config.apiKey || "");
