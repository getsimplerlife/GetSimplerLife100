...
export class Azure_blobClient {
  private client: HttpClient;
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.azure-blob.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createAzure_blobClient(config: ConnectionConfig): Azure_blobClient {
  const c = new Azure_blobClient(config.apiKey || "");
