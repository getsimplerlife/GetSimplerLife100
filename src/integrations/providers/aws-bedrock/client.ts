...
export class Aws_bedrockClient {
  private client: HttpClient;
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.aws-bedrock.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createAws_bedrockClient(config: ConnectionConfig): Aws_bedrockClient {
  const c = new Aws_bedrockClient(config.apiKey || "");
