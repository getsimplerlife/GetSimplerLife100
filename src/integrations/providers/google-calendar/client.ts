...
export class Google_calendarClient {
  private client: HttpClient;
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.google-calendar.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createGoogle_calendarClient(config: ConnectionConfig): Google_calendarClient {
  const c = new Google_calendarClient(config.apiKey || "");
