...
export class OpenaiClient {
  ...
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.openai.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createOpenaiClient(config: ConnectionConfig): OpenaiClient {
  const c = new OpenaiClient(config.apiKey || "");
