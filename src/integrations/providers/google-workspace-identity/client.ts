...
export class Google_workspace_identityClient {
  private client: HttpClient;
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.google-workspace-identity.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createGoogle_workspace_identityClient(config: ConnectionConfig): Google_workspace_identityClient {
  const c = new Google_workspace_identityClient(config.apiKey || "");
