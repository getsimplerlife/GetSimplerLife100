...
export class Azure_openaiClient {
  private client: HttpClient;
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.azure-openai.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createAzure_openaiClient(config: ConnectionConfig): Azure_openaiClient {
  const c = new Azure_openaiClient(config.apiKey || "");
