...
export class Microsoft_formsClient {
  private client: HttpClient;
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.microsoft-forms.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createMicrosoft_formsClient(config: ConnectionConfig): Microsoft_formsClient {
  const c = new Microsoft_formsClient(config.apiKey || "");
