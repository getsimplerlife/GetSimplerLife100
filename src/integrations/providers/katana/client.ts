...
export class KatanaClient {
  ...
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.katana.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createKatanaClient(config: ConnectionConfig): KatanaClient {
  const c = new KatanaClient(config.apiKey || "");
