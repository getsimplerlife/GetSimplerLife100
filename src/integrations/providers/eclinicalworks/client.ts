...
export class EclinicalworksClient {
  private client: HttpClient;
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.eclinicalworks.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createEclinicalworksClient(config: ConnectionConfig): EclinicalworksClient {
  const c = new EclinicalworksClient(config.apiKey || "");
