...
export class PaypalClient {
  ...
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.paypal.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createPaypalClient(config: ConnectionConfig): PaypalClient {
  const c = new PaypalClient(config.apiKey || "");
