...
export class TruckstopClient {
  ...
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.truckstop.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createTruckstopClient(config: ConnectionConfig): TruckstopClient {
  const c = new TruckstopClient(config.apiKey || "");
