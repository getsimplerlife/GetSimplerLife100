...
export class Anthropic_claudeClient {
  private client: HttpClient;
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.anthropic-claude.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createAnthropic_claudeClient(config: ConnectionConfig): Anthropic_claudeClient {
  const c = new Anthropic_claudeClient(config.apiKey || "");
