...
export class UipathClient {
  ...
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.uipath.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createUipathClient(config: ConnectionConfig): UipathClient {
  const c = new UipathClient(config.apiKey || "");
