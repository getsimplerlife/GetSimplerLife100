...
export class MistralClient {
  ...
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.mistral.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createMistralClient(config: ConnectionConfig): MistralClient {
  const c = new MistralClient(config.apiKey || "");
