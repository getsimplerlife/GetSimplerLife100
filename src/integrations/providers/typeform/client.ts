...
export class TypeformClient {
  ...
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.typeform.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createTypeformClient(config: ConnectionConfig): TypeformClient {
  const c = new TypeformClient(config.apiKey || "");
