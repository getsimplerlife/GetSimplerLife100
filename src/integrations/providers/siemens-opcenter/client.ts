...
export class Siemens_opcenterClient {
  private client: HttpClient;
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.siemens-opcenter.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createSiemens_opcenterClient(config: ConnectionConfig): Siemens_opcenterClient {
  const c = new Siemens_opcenterClient(config.apiKey || "");
