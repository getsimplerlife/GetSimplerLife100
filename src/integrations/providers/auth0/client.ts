...
export class Auth0Client {
  ...
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.auth0.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createAuth0Client(config: ConnectionConfig): Auth0Client {
  const c = new Auth0Client(config.apiKey || "");
