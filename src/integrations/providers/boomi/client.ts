...
export class BoomiClient {
  private client: HttpClient;
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.boomi.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createBoomiClient(config: ConnectionConfig): BoomiClient {
  const c = new BoomiClient(config.apiKey || "");
