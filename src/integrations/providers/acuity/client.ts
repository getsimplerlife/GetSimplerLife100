...
export class AcuityClient {
  ...
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.acuity.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createAcuityClient(config: ConnectionConfig): AcuityClient {
  const c = new AcuityClient(config.apiKey || "");
