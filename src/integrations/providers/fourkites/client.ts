...
export class FourkitesClient {
  private client: HttpClient;
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.fourkites.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createFourkitesClient(config: ConnectionConfig): FourkitesClient {
  const c = new FourkitesClient(config.apiKey || "");
