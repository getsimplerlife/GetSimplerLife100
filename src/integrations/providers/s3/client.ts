...
export class S3Client {
  ...
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.s3.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createS3Client(config: ConnectionConfig): S3Client {
  const c = new S3Client(config.apiKey || "");
