...
export class NextgenClient {
  ...
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.nextgen.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createNextgenClient(config: ConnectionConfig): NextgenClient {
  const c = new NextgenClient(config.apiKey || "");
