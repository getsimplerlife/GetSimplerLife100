...
export class Outlook_calendarClient {
  private client: HttpClient;
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.outlook-calendar.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createOutlook_calendarClient(config: ConnectionConfig): Outlook_calendarClient {
  const c = new Outlook_calendarClient(config.apiKey || "");
