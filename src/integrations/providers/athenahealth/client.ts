...
export class AthenahealthClient {
  ...
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.athenahealth.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createAthenahealthClient(config: ConnectionConfig): AthenahealthClient {
  const c = new AthenahealthClient(config.apiKey || "");
