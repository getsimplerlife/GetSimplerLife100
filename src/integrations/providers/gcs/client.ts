...
export class GcsClient {
  ...
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.gcs.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createGcsClient(config: ConnectionConfig): GcsClient {
  const c = new GcsClient(config.apiKey || "");
