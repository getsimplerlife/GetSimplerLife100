...
export class Authorize_netClient {
  private client: HttpClient;
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.authorize-net.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createAuthorize_netClient(config: ConnectionConfig): Authorize_netClient {
  const c = new Authorize_netClient(config.apiKey || "");
