...
export class Power_automateClient {
  private client: HttpClient;
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.power-automate.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createPower_automateClient(config: ConnectionConfig): Power_automateClient {
  const c = new Power_automateClient(config.apiKey || "");
