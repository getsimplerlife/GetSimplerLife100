...
export class Microsoft_bookingsClient {
  private client: HttpClient;
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.microsoft-bookings.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createMicrosoft_bookingsClient(config: ConnectionConfig): Microsoft_bookingsClient {
  const c = new Microsoft_bookingsClient(config.apiKey || "");
