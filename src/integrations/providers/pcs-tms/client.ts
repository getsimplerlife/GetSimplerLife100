...
export class Pcs_tmsClient {
  private client: HttpClient;
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.pcs-tms.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createPcs_tmsClient(config: ConnectionConfig): Pcs_tmsClient {
  const c = new Pcs_tmsClient(config.apiKey || "");
