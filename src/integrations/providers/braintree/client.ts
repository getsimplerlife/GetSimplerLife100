...
export class BraintreeClient {
  ...
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.braintree.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createBraintreeClient(config: ConnectionConfig): BraintreeClient {
  const c = new BraintreeClient(config.apiKey || "");
