...
export class WorkatoClient {
  ...
  constructor(apiKey: string) {
    this.client = new HttpClient({ baseUrl: "https://api.workato.com/v1", rateLimit: { maxRequestsPerSecond: 5 }, retry: { maxRetries: 3, baseDelay: 1000, maxDelay: 10000 }, timeout: 30000 });
  }
...
export function createWorkatoClient(config: ConnectionConfig): WorkatoClient {
  const c = new WorkatoClient(config.apiKey || "");
