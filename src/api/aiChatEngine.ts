...
export async function getOrCreateSession(
  ...
  const systemPrompt = isGuest ? GUEST_SYSTEM_PROMPT : "placeholder"; // Will be replaced per-request
...
export async function processChatMessage(
  ...
  if (!apiKey) {
    return {
      reply: "⚠️ AI chat service is not configured. Please contact support to enable the AI-powered assistant.",
      sessionId,
    };
  }
  ...
  // Prepare messages for OpenAI — always inject fresh system prompt
  // plus recent conversation history (last 20 messages to stay within context window)
  ...
  try {
    ...
    // Trim old messages to keep session manageable (keep last 50)
    if (session.messages.length > 50) {
      // Keep the first system message and last 49 messages
...
function generateFallbackReply(message: string, isGuest: boolean): string {
  ...
  if (lower.includes("price") || lower.includes("cost") || lower.includes("how much")) {
    return "Our AI agents start at **$750/month** per agent. We also offer implementation packages that deploy a team of agents for specific business functions. You can see all available plans on our billing page or visit the AI Employees Marketplace. Would you like more details on a specific package?";
  }

  if (lower.includes("employee") || lower.includes("agent") || lower.includes("ai team")) {
    return isGuest
      ? "We offer 18 types of AI employees spanning document processing, healthcare intake, invoice management, sales outreach, HR compliance, dispatch logistics, customer support, IT operations, financial planning, and more. Each agent is pre-trained for its industry and role. Which area interests you?"
      : "You can view all your AI employees in the **Employee Directory**. Active employees show their current tasks, status, and performance metrics. Deploy new agents from the **AI Employees Marketplace**.";
  }

  if (lower.includes("integration") || lower.includes("connect")) {
    return isGuest
      ? "Simpler Life 100 integrates with over 180 business platforms including Salesforce, HubSpot, QuickBooks, Slack, Gmail, Shopify, Jira, and many more. Our agents can read, write, and sync data across your entire tech stack."
      : "You can manage your integrations from the **Integrations** page. We support 180+ providers with OAuth connections. You can connect your CRM, email, accounting, and other tools.";
  }

  return isGuest
    ? "I'm here to help you learn about Simpler Life 100! You can ask me about our AI agents, pricing, integrations, or how we can automate your specific business processes. What would you like to know?"
